...
Consider a binary outcome as an illustration. Let $m$ represent the total number of providers, $n_i$ denote the number of subjects from provider $i$ $(i = 1, \dots, m)$, and $N := \sum_{i = 1}^m n_i$ signify the total number of records. For subject $j$ $(j = 1, \dots, n_i)$ from provider $i$, let $Y_{ij}$ be the outcome variable, and let $Z_{ij}$ constitute the vector of risk factors. The likelihood function can then be formulated as follows:
$$l(\boldsymbol{\gamma}, \boldsymbol{\beta}) \propto \sum_{i = 1}^m \sum_{j = 1}^{n_i}\{Y_{ij} \cdot (\gamma_i + Z_{ij}\boldsymbol{\beta}) - log(1 + e^{\gamma_i + Z_{ij}\boldsymbol{\beta}})\},$$ and the corresponding score function can be expressed as: $$U(\gamma_i) = \sum_{j = 1}^{n_i} \{Y_{ij} - \frac{e^{\gamma_i + Z_{ij}\boldsymbol{\beta}}}{1 + e^{\gamma_i + Z_{ij}\boldsymbol{\beta}}}\}$$ $$U(\beta_p) = \sum_{i = 1}^m \sum_{j = 1}^{n_i} \{Z_{ijp} \cdot (Y_{ij} - \frac{e^{\gamma_i + Z_{ij}\boldsymbol{\beta}}}{1 + e^{\gamma_i + Z_{ij}\boldsymbol{\beta}}})\}.$$ 
In the current context, the information matrix can be visualized as a $2 \times 2$ block matrix, expressed as:
$$ I(\boldsymbol{\gamma}, \boldsymbol{\beta}) = \begin{pmatrix} I(\boldsymbol{\gamma}) & I(\boldsymbol{\gamma}, \boldsymbol{\beta})   \\   I(\boldsymbol{\beta}, \boldsymbol{\gamma}) & I(\boldsymbol{\beta})  \end{pmatrix} \equiv \begin{pmatrix} I_{11} & I_{12} \\ I_{21} & I_{22}  \end{pmatrix}.$$ 
It is important to note that the $I(\boldsymbol{\gamma})$ block is "large" but diagonal. Consequently, we can compute $I^{-1}(\boldsymbol{\gamma}, \boldsymbol{\beta})$ as follows: $$I^{-1}(\boldsymbol{\gamma}, \boldsymbol{\beta}) = \begin{pmatrix} I_{11}^{-1} + {J_1}^{T} S^{-1} J_1 & - {J_2}^{T}   \\   - {J_2} &  S^{-1}  \end{pmatrix},$$ where $J_1 = I_{21}{I_{11}}^{-1}$, $S = I_{22} - J_1 I_{12}$, and $J_2 = S^{-1}J_1$.

The bottleneck in SerBIN lies in computing $I_{22}$ (i.e., $I(\boldsymbol{\beta})$) when the sample size $n$ is exceptionally large. However, each element of $I_{22}$ can be individually computed, allowing for parallel computing to enhance computational efficiency:
$$I(\boldsymbol{\beta})_{r,c} = \langle Z^{(r)}, Z^{(c)} \circ \mathbb{P} \rangle,$$ where $\langle a, b \rangle$ denotes the [inner product](https://en.wikipedia.org/wiki/Inner_product_space#:~:text=In%20mathematics%2C%20an%20inner%20product,angle%20brackets%20such%20as%20in) of $a$ and $b$ (i.e., $a^T b$), $\circ$ represents the [Hadamard product](https://en.wikipedia.org/wiki/Hadamard_product_(matrices)) (element-wise product), and $\mathbb{P} = \frac{e^{\gamma_i + Z_{ij}\boldsymbol{\beta}}}{(1 + e^{\gamma_i + Z_{ij}\boldsymbol{\beta}})^2}$. It's worth noting that $I(\boldsymbol{\beta})$ is symmetric, so there's a need to calculate only the upper triangular entries.
