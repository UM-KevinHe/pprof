...
List logis_firth_prov(arma::vec &Y, arma::mat &Z, arma::vec &n_prov, arma::vec gamma, arma::vec beta,
                      ...
                      double bound = 10.0, bool message = true, bool firth = true, bool gamma_tol = false) {
  if (message == true) {
  ...
  #pragma omp parallel shared(indices, iter, crit)
  {
    ...
    while (iter < max_iter && crit > tol) {
      ...
      while (true){
        ...
        } else {  // normal logistic regression
          TDS_YpA = TDS_Yp; // n_i*1
        }
      ...
      #pragma omp single
      {
        ...
        for (int i = 0; i < beta_size; ++i) {
          double abs_d_beta = std::fabs(d_beta[i]);
          if (abs_d_beta > max_val) {
            max_val = abs_d_beta;
            max_d_beta_id = i;
          }
        }
        double signed_crit = (max_d_beta_id >= 0) ? d_beta[max_d_beta_id] : 0.0;
        //double beta_id = max_d_beta_id + 1;

        if (message == true) {
          //cout << "Iter " << iter << ": Inf norm of running diff in beta is " << scientific << setprecision(4) << crit << ";" << endl;
          cout << "Iter " << iter << ": The component of d_beta with the largest magnitude is: " << scientific << setprecision(5) << signed_crit << ";" << endl;
          // cout << "Iter " << iter << ": The id of d_beta with the largest magnitude is: " << beta_id << ";" << endl;
        }
      }
      #pragma omp barrier
      ...
      #pragma omp single
      {
        ...
        if (gamma_tol == true){
          arma::vec d_gamma = gamma - old_gamma;
          crit2 = norm(d_gamma, "inf");
          crit = std::max(crit2, crit);
          if (message == true) {
            cout << "Iter " << iter << ": Inf norm of running diff in gamma is " << scientific << setprecision(4) << crit2 << ";" << endl;
          }
          old_gamma = gamma;
        }
      }
  ...
  if (message == true) {
    std::cout << "ParaFLR with " << threads <<  " cores converged after " << iter << " iterations." << endl;
  }
