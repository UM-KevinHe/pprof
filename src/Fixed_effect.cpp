...
// [[Rcpp::export]]
List logis_BIN_fe_prov2(arma::vec &Y, arma::mat &Z, arma::vec &n_prov, arma::vec gamma, arma::vec beta,
                       int parallel=1, int threads=1, double tol=1e-8, int max_iter=10000,
                       double bound=10.0, bool message = true, bool backtrack = false) {
  ...
  while (iter <= max_iter) {
    ...
    arma::mat mat_tmp1 = trans(info_betagamma.each_row()%info_gamma_inv.t());
    arma::mat schur_inv = inv_sympd(info_beta-mat_tmp1.t()*info_betagamma.t());
    arma::mat mat_tmp2_ori = mat_tmp1*schur_inv;
    arma::vec d_gamma_ori = info_gamma_inv%score_gamma + mat_tmp2_ori*(mat_tmp1.t()*score_gamma-score_beta);
    arma::vec d_beta_ori = schur_inv*score_beta - mat_tmp2_ori.t()*score_gamma;
    ...
    // Rcpp::Rcout << "Dimensions of mat_tmp2: " << mat_tmp2.n_rows << "x" << mat_tmp2.n_cols << std::endl;
    arma::vec d_gamma = info_gamma_inv % score_gamma +  mat_tmp2.t() * (mat_tmp1.t() * score_gamma - score_beta);
    // Rcpp::Rcout << "Dimensions of d_gamma: " << d_gamma.n_rows << "x1" << std::endl;
    arma::vec schur_system_solve = arma::solve(schur_system, score_beta, solve_opts::likely_sympd);
    // Rcpp::Rcout << "Dimensions of schur_system_solve: " << schur_system_solve.n_rows <<  "x" << schur_system_solve.n_cols << std::endl;
    // Rcpp::Rcout << "Dimensions of mat_tmp2: " << mat_tmp2.n_rows <<  "x" << mat_tmp2.n_cols << std::endl;
    // Rcpp::Rcout << "Dimensions of score_gamma: " << score_gamma.n_rows <<  "x" << score_gamma.n_cols << std::endl;
    arma::vec d_beta = schur_system_solve - mat_tmp2 * score_gamma;


    // List ret = List::create(_["d_beta"]=d_beta,
    //                         _["d_beta_ori"]=d_beta_ori);
    // return ret;
    //
