...
logis_fe <- function(data.prep, algorithm = "SerBIN", max.iter = 10000, tol = 1e-5, bound = 10,
  ...
  data <- data.prep$data
  N.char <- data.prep$char_list$N.char
  ...
  Y_bar <- sum(data[,Y.char])/sum(data[,N.char])
  gamma.prov <- rep(log(Y_bar/(1-Y_bar)), length(n.prov))
  beta <- rep(0, NCOL(Z))


  if (algorithm == "SerBIN") {
    if (Rcpp) { #Rcpp always use "backtrack"
      ls <- logis_BIN_fe_prov(as.matrix(data[,N.char]), as.matrix(data[,Y.char]), Z, n.prov, gamma.prov, beta,
      ...
      if (backtrack){ # initialize parameters for backtracking line search
        ...
        Loglkd <- function(gamma.obs, beta) {
          sum((gamma.obs + Z %*% beta) * data[, Y.char] - data[, N.char] * log(1 + exp(gamma.obs + Z %*% beta)))
        }
      }
      ...
      while (iter<=max.iter & beta.crit>=tol) {
        ...
        pq <- data[, N.char] * p*(1-p)
        pq[pq == 0] <- 1e-20
        score.gamma <- sapply(split(data[, Y.char] - data[, N.char] * p, data[, prov.char]), sum)
        score.beta <- t(Z) %*% (data[, Y.char] - data[, N.char] * p)
    ...
    if (Rcpp) {
      ls <- logis_fe_prov(as.matrix(data[, N.char]), as.matrix(data[, Y.char]), Z, n.prov, gamma.prov, beta, backtrack, max.iter, bound, tol, message)
      gamma.prov <- as.numeric(ls$gamma);
      beta <- as.numeric(ls$beta)
    } else {
      iter <- 0
      ...
      while (iter<=max.iter & beta.crit>=tol) {
        ...
        pq <- data[, N.char] * p * (1 - p)
        pq[pq == 0] <- 1e-20
        score.gamma.prov <- sapply(split(data[, Y.char] - data[, N.char] * p, data[, prov.char]), sum)
