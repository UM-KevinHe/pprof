...
SR_output <- function(fit, stdz = "indirect", measure = c("rate", "ratio"), null = "median",
  ...
  if (!class(fit) %in% c("logis_fe", "linear_fe")) stop("Object fit is not as required!", call.=F)
  ...
  if (class(fit) == "linear_fe") {
    if ("indirect" %in% stdz) {
      ...
      Exp.indirect_provider <- sapply(split(Exp, fit$prov), sum)
      Obs.indirect_provider <- sapply(split(fit$obs, fit$prov), sum)

      indirect_stdz.diff <- matrix((Obs.indirect_provider - Exp.indirect_provider)/n.prov)
      dimnames(indirect_stdz.diff) <- list(rownames(fit$gamma), "Indirect_standardized.difference")
      return_ls$indirect.difference <- indirect_stdz.diff
    }

    if ("direct" %in% stdz) {
      n <- nrow(fit$data_include)
      Obs.direct_provider <- sum(fit$obs)
      Exp.direct <- function(gamma){
        sum(gamma + Z_beta)
      }
      Exp.direct_provider <- sapply(gamma.prov, Exp.direct)
      direct_stdz.diff <- matrix((Obs.direct_provider - Exp.direct_provider)/n)
      dimnames(direct_stdz.diff) <- list(rownames(fit$gamma), "Direct_standardized.difference")
